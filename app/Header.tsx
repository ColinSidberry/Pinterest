  ...
  return (
    <header className='flex items-center justify-between p-6'>
      <div className="flex items-center gap-3 flex-1">
        <PinterestLogo />
        <NavItems status={status} pathname={pathname} handleSignOut={handleSignOut} handleSignIn={handleSignIn} />
        <SearchBar />
      </div>
      <MobileMenu pathname={pathname} isMenuOpen={isMenuOpen} setIsMenuOpen={setIsMenuOpen} handleMenuClick={handleMenuClick} />
      <DesktopMenu pathname={pathname} handleMenuClick={handleMenuClick} />
    </header>
  );
}
...
function NavItems({ status, pathname, handleSignOut, handleSignIn }: NavItemsProps) {
  if (status === "authenticated") {
    return (
      <>
        <Link href="/home" passHref>
          <button className={`font-semibold p-2 px-4 rounded-full ${pathname === '/home' ? 'bg-black text-white' : ''}`}>
            Home
          </button>
        </Link>
        <button className='font-semibold p-2 px-4 rounded-full'>Create</button>
        <button className='font-semibold p-2 px-4 rounded-full whitespace-nowrap' onClick={handleSignOut}>
          Sign Out
        </button>
      </>
    );
  ...
  return (
    <button className='font-semibold p-2 px-4 rounded-full whitespace-nowrap' onClick={handleSignIn}>
      Sign In
    </button>
  );
}
...
interface MobileMenuProps {
  ...
  handleMenuClick: (link: string) => Promise<void>;
}

function MobileMenu({ pathname, isMenuOpen, setIsMenuOpen, handleMenuClick }: MobileMenuProps) {
  return (
...
function DesktopMenu({ pathname, handleMenuClick }: DesktopMenuProps) {
  return (
    <div className="hidden md:flex">
      ...
      <button className='font-semibold p-2 px-4 rounded-full' onClick={() => handleMenuClick(LINKEDIN)}>
        About
      </button>
      <button className='font-semibold p-2 px-4 rounded-full' onClick={() => handleMenuClick(MEDIUM)}>
        Blog
      </button>
    </div>
  );
